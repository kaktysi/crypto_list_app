...
class CryptoCoinsRepository implements AbstractCoinsRepository {
  ...
  Future<List<CryptoCoin>> _fetchCoinsListFromApi() async {
    final response = await dio.get(
      'https://min-api.cryptocompare.com/data/pricemultifull?fsyms=BTC,ETH,BNB,AVAX,AID,SOL,HAM,HMSTR,TON,DOGE,USDT,NOT,DOGS,HMND&tsyms=USD'
    );
  ...
  Future<CryptoCoin> getCoinDetails(String currencyCode) async {
    try {
      final coin = await _fetchCryptoCoinFromApi(currencyCode);
  ...
  Future<CryptoCoin> _fetchCryptoCoinFromApi(String currencyCode) async {
    final response = await dio.get(
